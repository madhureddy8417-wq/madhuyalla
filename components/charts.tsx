...
import type { SuitableCropInfo, CropMarketData } from '../types';
    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-white p-2 border border-gray-300 rounded shadow-lg">
                ...
                <p className="text-sm text-green-600">{`${payload[0].name}: ${payload[0].value}`}</p>
            </div>
        );
    }
    return null;
...
export const CropYieldChart: React.FC<{ data: SuitableCropInfo[] }> = ({ data }) => {
    // Defer access to window.Recharts until render time to avoid race condition
    ...
    const chartData = data.map(crop => ({
        name: crop.cropName,
        'Potential Yield': crop.potentialYieldValue,
    }));

    return (
        <div style={{ width: '100%', height: 300 }}>
            <h4 className="text-lg font-bold text-green-800 text-center mb-2">Potential Yield</h4>
            <ResponsiveContainer>
                <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    ...
                    <Legend />
                    <Bar dataKey="Potential Yield" fill="#34D399" />
                </BarChart>
            </ResponsiveContainer>
        </div>
    );
...
export const CropPriceChart: React.FC<{ data: SuitableCropInfo[] }> = ({ data }) => {
    if (typeof window.Recharts === 'undefined') {
        return <div style={{height: 300}} className="flex items-center justify-center text-gray-500 mt-8">Loading Chart...</div>;
    }
    const { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } = window.Recharts;

    const chartData = data.map(crop => ({
        name: crop.cropName,
        'Avg Market Price (₹)': crop.avgMarketPriceValue,
    }));

    return (
        <div style={{ width: '100%', height: 300 }} className="mt-8">
            <h4 className="text-lg font-bold text-green-800 text-center mb-2">Average Market Price</h4>
            <ResponsiveContainer>
                <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" angle={-45} textAnchor="end" height={60} interval={0} />
                    ...
                    <Tooltip content={<CustomTooltip />} />
                    <Legend />
                    <Bar dataKey="Avg Market Price (₹)" fill="#3B82F6" />
                </BarChart>
            </ResponsiveContainer>
        </div>
    );
